local mod	= DBM:NewMod("LichKing", "DBM-Icecrown", 5) --Edit by Lightball
...
function mod:OldDefileTarget()
	...
	if targetname == UnitName("player") then
		...
		if self.Options.YellOnDefile then
			SendChatMessage(L.YellDefile:format(targetname), "SAY")
		end
...
function mod:OldTankTrap()
	...
	if LKTank == UnitName("player") then
		...
		if self.Options.YellOnTrap then
			SendChatMessage(L.YellTrap:format(LKTank), "SAY")
		end
	end
...
function mod:OldTrapTarget()
	...
	if targetname ~= LKTank then--If scan doesn't return tank abort other scans and do other warnings.
		...
		if targetname == UnitName("player") then
			specWarnTrap:Show()
			if self.Options.YellOnTrap then
				SendChatMessage(L.YellTrap:format(targetname), "SAY")
			end
		end
		local uId = DBM:GetRaidUnitId(targetname)
		if uId then
...
function mod:SPELL_CAST_START(args)
	...
	elseif args:IsSpellID(70498) then -- Vile Spirits
		...
		if not timerHarvestSoulCD:IsStarted() then
			timerHarvestSoulCD:Start(50)
		end
	...
	elseif args:IsSpellID(73650) then -- Restore Soul (Heroic)
		...
		timerDefileCD:Start(45) -- Defile after FMC
...
function mod:SPELL_CAST_SUCCESS(args)
	...
	elseif args:IsSpellID(68980, 74325, 74326, 74327) then -- Harvest Soul
		...
		timerHarvestSoul:Start(107) -- Next Harvest Soul. 1sec off?
		timerDefile:Start(52) -- Defile After FMC
		if args:IsPlayer() then
	...
	elseif args:IsSpellID(73654, 74295, 74296, 74297) then -- Harvest Souls (Heroic)
		...
		timerHarvestSoul:Start(107)
		timerDefile:Start(52)
		warnDefileSoon:Schedule(47)
		timerVileSpirit:Cancel()
		timerSoulreaperCD:Cancel()
		-- timerDefileCD:Cancel() Dodano novi TEST
		warnDefileSoon:Cancel()
	end
end
...
do
	...
	function mod:SPELL_AURA_APPLIED(args)
		...
		elseif args:IsSpellID(73650) and time() - lastRestore > 3 then		-- Restore Soul (Heroic)
			...
			timerDefileCD:Start(65)-- #FidozEdit: this simply adds a defile timer
		end
	end
end
			...
			for i=0, GetNumRaidMembers() do        -- for every raid member check ..
				if UnitInVehicle("raid"..i) and not valkyrTargets[i] then      -- if person #i is in a vehicle and not already announced 
					...
					if UnitName("raid"..i) == UnitName("player") then
						...
						SendChatMessage(L.YellValk:format(UnitName("player")), "SAY") --Say own name when Valkd
					  --SendChatMessage(L.YellDefile:format(target), "SAY") example
...
function mod:NextPhase()
	...
	elseif phase == 2 then --Timers corrected to Warmane scripts
		timerSummonValkyr:Start(26.5)
		timerSoulreaperCD:Start(46.5)
		timerDefileCD:Start(44.5) -- #FidozEdit from 37 to 44.5
		timerInfestCD:Start(20.5) 
		warnDefileSoon:Schedule(39.5)
	elseif phase == 3 then
		timerVileSpirit:Start(20)
		timerSoulreaperCD:Start(50)
		-- timerDefileCD:Start(38)
		timerHarvestSoulCD:Start(14.5) -- #FidozEdit: I have not changed this, but this is wrong. --Fixed: was 20
	...
	elseif msg == "TrapOn" then
		if not self.Options.LKBugWorkaround then
			...
			if target == UnitName("player") then
				specWarnTrap:Show()
				if self.Options.YellOnTrap then
					SendChatMessage(L.YellTrap:format(target), "SAY")
				end
			end
	...
	elseif msg == "DefileOn" then
		if not self.Options.LKBugWorkaround then
			...
			if target == UnitName("player") then
				...
				soundDefile:Play()
				if self.Options.YellOnDefile then
					SendChatMessage(L.YellDefile:format(target), "SAY")  -- SendChatMessage(L.YellDefile, "SAY")
				end
			elseif target then
				local uId = DBM:GetRaidUnitId(target)
				if uId then
